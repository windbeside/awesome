...
def _gen_sql(tbl_name, mappings):
    pk = None
    sql = ['-- generating SQL for %s:' % tbl_name, 'create table `%s` (' % tbl_name ]
        ...
        for k, v in attrs.iteritems():
            if isinstance(v, Field):
                ...
                if v.primary_key:
                    ...
                    if v.updatable:
                        logging.warning('NOTE: Change primary key non-updatable.')
                        v.updatable = False
                    if v.nullable:
                        logging.warning('NOTE: Change primary key non-nullable.')
                        v.nullable = False
                    primary_key = v
                mappings[k] = v
        #是否存在主键
...
class Model(dict):
    ...
    def __getattr__(self, key):
        try:
            ...
            raise ("'Dict' object has no attribute '%s'" % key)
